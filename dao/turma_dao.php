<?php 
    ...
    class TurmaDAO {
        ...
        private function mapTurmas($resultSql) {
            $turmas = array();
            foreach ($resultSql as $reg):
                $turma = new Turma();
                $turma->setId_turma($reg['id_turma']);
                $turma->setCodigo_disciplina($reg['codigo_disciplina']);
                $turma->setNome_disciplina($reg['nome_disciplina']);
        ...
        public function findById($idTurma) {
            ...
            $sql = TurmaDAO::SQL_TURMA." WHERE t.id_turma = ?";
        ...
        public function save(Turma $turma) {
            ...
            $sql = "INSERT INTO turmas (codigo_disciplina, nome_disciplina, ano, id_curso)".
                " VALUES (?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$turma->getCodigo_disciplina(), $turma->getNome_disciplina(), 
        ...
        public function update(Turma $turma) {
            ...
            $sql = "UPDATE turmas SET codigo_disciplina = ?, nome_disciplina = ?,". 
                " ano = ?, id_curso = ?".
                " WHERE id_turma = ?";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$turma->getCodigo_disciplina(), $turma->getNome_disciplina(), 
        ...
        public function delete(Turma $turma) {
            ...
            $stmt = $conn->prepare($sql);
            $stmt->execute([$turma->getId_turma()]);
        }
    }
?>